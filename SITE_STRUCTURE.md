...
トップページのメインビジュアルは、活動写真の上にアイコンなしの白い `Molkkynist + since 2024` ワードマーク画像を直接重ねる。写真を白くぼかす加工は使わず、写真上部には緑帯のコピー「モルックで繋がるコミュニティ」を配置する。説明文と参加CTAはメインビジュアル下に分離する。スクロール時はヒーロー写真を画面上に固定表示し、ヒーロー紹介エリアが写真の上に重なっていく見せ方にする。ヒーロー紹介エリアには、装飾用の横スクロール画像ストリームを置き、活動のにぎわいを軽く補足する。現時点では実写真ではなく淡いプレースホルダーで構成し、`prefers-reduced-motion` ではアニメーションを停止する。
...
- トップページの左上ロゴ付きヘッダーを表示しない構成へ変更。右上ハンバーガーはスマートフォンで維持し、ヒーロー写真上部に緑帯コピー「モルックで繋がるコミュニティ」を追加した。
