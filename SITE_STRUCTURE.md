...
- お知らせ（正方形ビジュアル＋日付＋タイトルのニュースカードを横スライドするカルーセル `report-carousel` / `report-news-card`。PC は3枚グリッド / タブレットは2枚グリッド / SP は中央1枚＋両隣ピーク表示。SPでは両隣カードのタップでそのカードに送る。5秒間隔で自動送り、前後ボタン・ドットインジケータは設けない。各カードは共通テンプレート `news.html` に `?id=xxx` を付けて遷移する。表示データは将来 Firestore `news` から取得する想定）
...
- `index.html` の `#about` と `#schedule` の間に `#news`（お知らせ）セクションを新設。
  - UI は `report-carousel` / `report-news-card` のカルーセル形式で、正方形ビジュアル＋日付（小・左寄せ）＋タイトルのカードを横スライド・自動送りで表示する。
