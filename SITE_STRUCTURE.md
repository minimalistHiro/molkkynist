...
- 開催スケジュール（月次カレンダー表示。Firestore `events` の公開済みイベントを当該日にハイライトし、カレンダー下に当月のイベント一覧を表示する。`events.html` への導線ボタンは廃止。カレンダー下に「開催場所について詳しくはこちら」のリンクボタンを置き、`venues.html` へ遷移させる）
