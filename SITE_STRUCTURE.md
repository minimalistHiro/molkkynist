...
- 次回イベント / 開催スケジュール（月次カレンダー表示。Firestore `events` の公開済みイベントを当該日にハイライトし、カレンダー下に当月のイベント一覧を表示する。`events.html` への導線ボタンは廃止。カレンダー下に「開催場所について詳しくはこちら」の展開ボタンを置き、開催場所名・住所・Googleマップ確認ボタンを一覧表示する）
