...
- モルックとは（3つの紹介文ブロックごとに実写3枚を組み合わせた左右反転コラージュを配置し、末尾の横長キャラクターイラストと合わせて概要を伝える。詳しい基本ルールは別ページ `molkky.html` に残す）
...
- フッター直前のSNS導線（緑背景に `follow us` ラベル、公式Instagramリンク、X の仮リンク、公式LINEリンクを置く。大きな参加見出し、説明文、フォーム申込ボタンは置かない）

トップページのメインビジュアルは、活動写真の上にアイコンなしの白い `Molkkynist + since 2024` ワードマーク画像を直接重ねる。写真を白くぼかす加工は使わず、写真上部には緑帯のコピー「モルックから始まる、最高の縁」を配置する。説明文と参加CTAはメインビジュアル下に分離する。スクロール時はヒーロー写真を画面上に固定表示し、ヒーロー紹介エリアが写真の上に重なっていく見せ方にする。ヒーロー紹介エリアには、`assets/images/hero/stream/slide-01.jpg` 〜 `slide-15.jpg` の活動写真15枚を横方向へ連続表示する装飾用画像ストリームを置き、活動のにぎわいを軽く補足する。画像は正方形枠内で中央トリミングし、`prefers-reduced-motion` ではアニメーションを停止する。

ヘッダーナビゲーション:

- デスクトップでは横並びナビゲーションを表示する。トップページではメインビジュアルの見え方を優先し、デスクトップのヘッダーは表示しない。
- スマートフォンでは右上のハンバーガーボタンから、`モルックとは`、`開催スケジュール`、`参加の流れ`、`活動の様子`、`運営メンバー`、`よくある質問`、`参加・お問い合わせ` を縦並びで表示する。
...
- 個別メンバー詳細の下部には、公開メンバー全員を表示順どおりに横スライド表示する「他の運営メンバー」導線を置く。メンバーが2人なら `A B A B...`、3人なら `A B C A B C...` のように、余白を空けずに永久ループで連続表示する。
...
- `contact-complete.html` はフォーム送信成功後にだけ遷移する送信完了ページとして扱う

主な掲載内容:

- Instagram DM へのリンク
- お問い合わせフォーム（必須: 名前・メール／任意: 電話番号／用件区分: 参加希望・イベント問い合わせ・メディア取材・その他／一言／参加希望時は Firestore `events` から取得した開催日程を1件のみトグル選択し、選択中イベントの開催日時・開催場所・雨天時対応・会場画像を一言欄の下に表示する。会場住所はGoogleマップへ遷移できるリンクにする）
- 送信完了ページ（フォーム送信完了の明示、受付完了メール送信、3営業日以内を目安に返信する旨、公式Instagram DM補足。参加希望で送信した場合は、予約したイベントの開催日時・開催場所・雨天時対応・会場画像を再表示する）
- 見学や初参加に関する案内
...
- `contactSubmissions.inquiryType == "participate"` の参加希望は非表示にし、イベント参加者一覧で扱う。
...
- 今後開催予定のイベントを、トップページの開催スケジュールに近いカード形式で表示
- 各イベントカードに参加人数を表示
- イベントカード選択後、対象イベント専用の参加者一覧ページへ遷移
- 詳細ページでは参加者名のみを初期表示
- 各参加者名の右側にあるくの字型ボタンを開くと、メールアドレス、電話番号、一言・ご質問、申込日時、対応ステータスを表示

備考:

- 参加者情報は `contactSubmissions` の参加希望データから集計する。
- 管理者専用画面のため、公開サイトには導線を出さない。
...
- メンバー一覧（`index.html#members`）のカード仕様を変更。
  ...
  - 詳細データは `js/member-data.js` のローカルデータを正本として表示する。現行の公開メンバーIDは `ishii`。
...
- `index.html` のヒーロー紹介エリアと `#about`（モルックとは）セクションの境目を、直線ではなく1周期分の大きな波形に変更。
  - 1段目と3段目は左に本文・右に写真群、2段目は左に写真群・右に本文を配置する。
...
- 全公開ページ共通ヘッダーの `index.html#about` へのナビゲーション文言を「Molkkynistについて」から「モルックとは」に変更し、トップページ `#about` の見出しと意味を揃えた。
- トップページの左上ロゴ付きヘッダーを表示しない構成へ変更。右上ハンバーガーはスマートフォンで維持し、ヒーロー写真上部に緑帯コピー「モルックから始まる、最高の縁」を追加した。
