...
- 開催スケジュール（月次カレンダー表示。Firestore `events` の公開済みイベントを当該日にハイライトし、カレンダー下に当月のイベント一覧を表示する。`events.html` への導線ボタンは廃止。カレンダー下に開催場所の詳細を直接表示し、別ページへ遷移しなくても会場情報を確認できるようにする）
